-- given MISC ref des don't have a component placement in BOM, we skip BOM tables and look at the CAUSE BY for placement
SELECT 
-- business keys
...
null "BOM Default Component Placed At", --TO_NUMBER(REGEXP_SUBSTR(DD.CHARACTER24,'/\s*([0-9]+)',1,1,NULL,1)) "BOM Default Component Placed At", -- 24
TO_NUMBER(REGEXP_SUBSTR(DD.CHARACTER24,'/\s*([0-9]+)',1,1,NULL,1)) "Component Placed At", --25
...
(
  select 
character2 as code_desc,
character3 as process_group,
character5 as active,
character6 as comments,
character92 as org,
character93 as code
from qa_results x
where plan_id = 143
and character92 ='NSS'
and character3 = 'DEFECT-REF'
and character5 = 'Y'
) miscxx
on dd.plan_id = 5166 -- defect details
and dd.character20 = miscxx.code
...
LEFT JOIN MTL_SYSTEM_ITEMS_B         MSIAC
       ...
       AND RR.FROM_OP_SEQ_NUM =
           (SELECT MIN (RR2.from_op_seq_num)
             ...
             WHERE     RR2.PLAN_ID = RR.PLAN_ID
                   ...
                   AND RR2.FROM_OP_SEQ_NUM >= TO_NUMBER(REGEXP_SUBSTR(DD.CHARACTER24,'/\s*([0-9]+)',1,1,NULL,1))
                        )
       AND EXISTS
