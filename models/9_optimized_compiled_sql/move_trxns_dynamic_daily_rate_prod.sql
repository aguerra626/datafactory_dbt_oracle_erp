...
WHERE
  ...
  AND first_unit_start_date BETWEEN SYSDATE - 365
  AND SYSDATE + 365
...
),  dbt__cte__stg_from_wop_we_wdj_msib__ as (
SELECT
    src_fm_wop.*,
    stg_we_wdj_msib.*,
...
),  dbt__cte__stg_from_wop_we_wdj_msib_lf_wmt__ as (
SELECT
    *
FROM
    dbt__cte__stg_from_wop_we_wdj_msib__
    stg_base
...
FROM
    dbt__cte__stg_from_wop_we_wdj_msib_lf_wmt__
    int_base
WHERE
