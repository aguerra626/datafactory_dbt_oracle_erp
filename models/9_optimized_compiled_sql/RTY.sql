...
WHERE
    ...
    AND WMT.TRANSACTION_DATE >= '01-JAN-2025'
    AND WMT.SCRAP_ACCOUNT_ID IS NULL -- Remove Scrap Qtys
    AND MS.SEGMENT1 IN ('987-0018-661', '987-0018-661+PR')
    -- per customer request excluding , '966-4518-006'
GROUP BY
...
SELECT
    TRUNC (WMT.TRANSACTION_DATE, 'MM') AS "FIRST MONTH DATE"
  , WMT.FM_OPERATION_SEQ_NUM           AS "FM OP SEQ NUM RTY"
  , MS.SEGMENT1                        AS "PART NUMBER RTY"
  , SUM (WMT.TRANSACTION_QUANTITY)     AS "TRANSACTION QTY RTY"
  , 'RTY_CONFIRMATIONS'                AS FEATURE
FROM
    WIP.WIP_MOVE_TRANSACTIONS     WMT
    JOIN WIP.WIP_OPERATIONS FMWOP
...
WHERE
    ...
    AND WMT.TRANSACTION_DATE >= '01-JAN-2025'
    AND WMT.SCRAP_ACCOUNT_ID IS NULL -- Remove Scrap Qtys
    AND MS.SEGMENT1 IN ('966-4516-007')
    AND WMT.FM_OPERATION_SEQ_NUM != 500
...
WHERE
    ...
    AND QR.QA_CREATION_DATE >= '01-JAN-2025'
    ...
    AND MSIK.SEGMENT1 IN ('987-0018-661', '987-0018-661+PR')
    -- per customer request excluding , '966-4518-006'
GROUP BY
...
WHERE
    ...
    AND PCH.PARENT_COLLECTION_ID =QRESULT.COLLECTION_ID
    AND PCH.PARENT_OCCURRENCE = QRESULT.OCCURRENCE
    AND MSIK.SEGMENT1 IN ('966-4516-007')
    AND QR.FROM_OP_SEQ_NUM != 500
    -- per customer request excluding , '966-4518-006'
GROUP BY
    TRUNC (QR.QA_CREATION_DATE, 'MM')
  , QR.FROM_OP_SEQ_NUM
...
WHERE
    ...
    AND WMT.TRANSACTION_DATE >= '01-JAN-2025'
    AND WMT.SCRAP_ACCOUNT_ID IS NULL -- Remove Scrap Qtys
    AND (MS.SEGMENT1 IN ('987-0018-661', '987-0018-661+PR', '966-4518-006') OR MS.SEGMENT1 LIKE '966-2083%')
GROUP BY
    TRUNC (WMT.TRANSACTION_DATE, 'MM')
  , WMT.FM_OPERATION_SEQ_NUM
  , MS.SEGMENT1
...
WHERE
    ...
    AND PCH.PARENT_COLLECTION_ID =QRESULT.COLLECTION_ID
    AND PCH.PARENT_OCCURRENCE = QRESULT.OCCURRENCE
    AND (MSIK.SEGMENT1 IN ('987-0018-661', '987-0018-661+PR', '966-4518-006') OR MSIK.SEGMENT1 LIKE '966-2083%')
GROUP BY
    TRUNC (QR.QA_CREATION_DATE, 'MM')
  , QR.FROM_OP_SEQ_NUM
  , MSIK.SEGMENT1
...
FROM
    (
    SELECT
        RTY_FOREACH."PART NUMBER RTY"
      , RTY_FOREACH."FIRST MONTH DATE"
      , RTY_FOREACH."FM OP SEQ NUM RTY"
      , NVL (RTY_FOREACH.RTY_CONFIRMATIONS_SUM_QTY, 0)                                                AS TOT_CONFIRMS
      , NVL (RTY_FOREACH.RTY_DEFECTS_SUM_QTY, 0)                                                      AS TOT_DEFECTS
      , ROUND (NVL (RTY_FOREACH.RTY_CONFIRMATIONS_SUM_QTY, 0) 
      / (NVL (RTY_FOREACH.RTY_CONFIRMATIONS_SUM_QTY, 0) + NVL (RTY_FOREACH.RTY_DEFECTS_SUM_QTY, 0) ) , 4) AS YIELD_RATIO
    FROM
        RTY_FOREACH
    ) RTY 
) RESULTS_FOREACH
WHERE 1=1
    AND RESULTS_FOREACH."FM OP SEQ NUM RTY" = 1000
...
(
SELECT
    RTY_COMBINED."PART NUMBER RTY"
  , RTY_COMBINED."FIRST MONTH DATE"
  , RTY_COMBINED."FM OP SEQ NUM RTY"
  , NVL (RTY_COMBINED.RTY_CONFIRMATIONS_SUM_QTY, 0)                                                AS TOT_CONFIRMS
  , NVL (RTY_COMBINED.RTY_DEFECTS_SUM_QTY, 0)                                                      AS TOT_DEFECTS
  , ROUND (NVL (RTY_COMBINED.RTY_CONFIRMATIONS_SUM_QTY, 0) 
  / (NVL (RTY_COMBINED.RTY_CONFIRMATIONS_SUM_QTY, 0) + NVL (RTY_COMBINED.RTY_DEFECTS_SUM_QTY, 0) ) , 4) AS YIELD_RATIO
FROM
    RTY_COMBINED
) RTY 
) RESULTS_COMBINED
WHERE 1=1
    AND RESULTS_COMBINED."FM OP SEQ NUM RTY" = 1000