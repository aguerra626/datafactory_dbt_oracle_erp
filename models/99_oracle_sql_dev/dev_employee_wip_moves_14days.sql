...
),  dbt__cte__src_wip_move_transactions_14days__ as (
SELECT
...
),  dbt__cte__stg_we_wdj_wmt14days__ as (
SELECT
    src_we.*,
    src_wdj.*,
    src_wmt.*
...
FROM
    ...
    JOIN dbt__cte__src_wip_move_transactions_14days__
    src_wmt
...
WHERE
  organization_id = 1213
  and first_unit_start_date >= sysdate - 365
),  dbt__cte__stg_we_wdj_wmt14days_fm_to_wop__ as (
SELECT
    stg_we_wdj_wmt.*,
    fmwop.*,
    towop.*
FROM
    dbt__cte__stg_we_wdj_wmt14days__
    stg_we_wdj_wmt
...
WHERE
    organization_id = 1213
),  dbt__cte__stg_we_wdj_wmt14days_fm_to_wop_fm_to_bd__ as (
SELECT
...
FROM
    dbt__cte__stg_we_wdj_wmt14days_fm_to_wop__
    stg_we_wdj_wmt_fm_to_wop
...
),  dbt__cte__stg_we_wdj_wmt14days_fm_to_wop_fm_to_bd_msib_mp__ as (
SELECT
    stg_we_wdj_wmt_fm_to_wop_fm_to_bd.*,
    stg_msib_mp.*
FROM
    dbt__cte__stg_we_wdj_wmt14days_fm_to_wop_fm_to_bd__
    stg_we_wdj_wmt_fm_to_wop_fm_to_bd
...
),  dbt__cte__int_employee_wip_moves_14days__ as (
select
int_base.*,
...
src_fu.*
from dbt__cte__stg_we_wdj_wmt14days_fm_to_wop_fm_to_bd_msib_mp__ int_base
...
from dbt__cte__int_employee_wip_moves_14days__ int_base
)
select * from base